export const generateCodePrompt = ({
  ...
  const isLocalCsvPath = Boolean(
    csvFileUrl && /^\/?[^:]+\.csv$/i.test(csvFileUrl)
  );
...
${
  isLocalCsvPath
    ? `The dataset is available at the following local file path inside the sandbox: ${
        csvFileUrl || "[NO FILE URL PROVIDED]"
      }`
    : `The dataset is available at the following file URL: ${
        csvFileUrl || "[NO FILE URL PROVIDED]"
      }`
}
...
- ${
    isLocalCsvPath
      ? "Reads the CSV directly from the provided local path. Do not use requests or storage_options for local sandbox files."
      : "Downloads the CSV from the provided file URL. Note: Because file hosting CDNs (like utfs.io) block standard python/pandas User-Agents with 403 Forbidden, you MUST always pass a browser-like User-Agent header when downloading. For example, if using pandas: pd.read_csv(url, storage_options={'User-Agent': 'Mozilla/5.0'}) or if using requests: requests.get(url, headers={'User-Agent': 'Mozilla/5.0'})."
  }
...
- Always wrap the python code in markdown code blocks with the language specifier: \`\`\`python ... \`\`\`
